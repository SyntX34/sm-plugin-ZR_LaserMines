...
{   
    ...
    g_smLasermineToSteamID = new StringMap();
...
public void OnLibraryAdded(const char[] name)
{
    if (StrEqual(name, "ZombieRiot"))
    {
        ...
        if (g_bDebug) LogMessage("[Lasermines] Zombie:Riot detected");
    }
    else if (StrEqual(name, "zombiereloaded"))
    {
        g_bHasZombieReloaded = true;
        if (g_bDebug) LogMessage("[Lasermines] Zombie:Reloaded detected");
    }
...
public void OnMapEnd()
{
    ...
    StringMapSnapshot snapshot = g_smLasermineToSteamID.Snapshot();
    for(int i = 0; i < snapshot.Length; i++)
    {
        snapshot.GetKey(i, key, sizeof(key));
        entity = StringToInt(key);
    ...
    {
        g_iClientsAmount[i] = 0;
        g_iClientsMyAmount[i] = 0;
        g_iClientsMaxLimit[i] = 0;
        g_iUsedByNative[i] = false;
        g_iClientLasermineCount[i] = 0;
    }
    if(g_bDebug)
    {
        LogMessage("[Lasermines Debug] Map ended, cleared all mines and reset client data");
    }
}
...
public void OnConfigsExecuted()
{
    ...
    if(g_bDebug)
    {
        LogMessage("[Lasermines] Config loaded - Amount: %d, MaxAmount: %d, Damage: %d", 
...
public void OnRoundEnd(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if(g_bDebug)
    {
        LogMessage("[Lasermines Debug] Round ended, cleared all mines");
    }
}
...
public void OnClientDisconnect(int client)
{
    ...
    {
        LogMessage("[Lasermines Debug] Client %N disconnected, mines persist for reconnection", client);
    }
}
...
void ClearClientLasermines(int client)
{
    ...
    {
        if(g_bDebug) LogMessage("[Lasermines Debug] Failed to get SteamID for client %N", client);
        return;
    }
    ...
    if(g_bDebug)
    {
        StringMapSnapshot debugSnapshot = g_smLasermineToSteamID.Snapshot();
        LogMessage("[Lasermines Debug] StringMap has %d total entries", debugSnapshot.Length);
        for(int i = 0; i < debugSnapshot.Length; i++)
        {
            debugSnapshot.GetKey(i, key, sizeof(key));
            char ownerSteamID[32];
            if(g_smLasermineToSteamID.GetString(key, ownerSteamID, sizeof(ownerSteamID)))
            {
                LogMessage("[Lasermines Debug] StringMap entry: mine %s -> steam %s", key, ownerSteamID);
            }
        }
        delete debugSnapshot;
    }
    ...
    for(int i = 0; i < snapshot.Length; i++)
    {
        ...
        if(g_smLasermineToSteamID.GetString(key, ownerSteamID, sizeof(ownerSteamID)))
        {
            if (StrEqual(ownerSteamID, steamID))
            {
                ...
                {
                    found++;
                    if(g_bDebug) LogMessage("[Lasermines Debug] Found mine %d belonging to steam %s, marking for removal", entity, steamID);
                    toRemove.Push(entity);
                }
            }
        }
    ...
    if(g_bDebug) LogMessage("[Lasermines Debug] Found %d mines for client %N (steam %s) to remove", found, client, steamID);
    ...
    for(int i = 0; i < toRemove.Length; i++)
    {
        ...
        if(IsValidEntity(entity))
        {
            if(g_bDebug) LogMessage("[Lasermines Debug] Removing mine %d", entity);
            RemoveLasermine(entity);
        }
    }
...
void RemoveLasermine(int lasermine)
{
    ...
    g_smLasermineToSteamID.Remove(key);
    if(beam != -1)
    {
...
bool IsClientProtected(int client)
{
    if (g_bHasZombieRiot)
    {
        return ZRiot_IsClientProtected(client);
    }
    else if (g_bHasZombieReloaded)
    {
        return false;
...
int GetClientMineCount(int client)
{
    char steamID[32];
    if (!GetClientAuthId(client, AuthId_Steam2, steamID, sizeof(steamID)))
        return 0;
...
void HandleClientBecomeZombie(int client)
{
    switch(g_iDeathBehavior)
    {
        case 0:
        {
            // do nothing
        ...
        case 1:
        {
            ...
            if(g_bDebug && g_cvDebug.IntValue >= 2)
            {
                LogMessage("[Lasermines Debug] Client %N became zombie, removed all planted mines (behavior 1)", client);
            }
        }
        case 2:
        {
            ...
            if(g_bDebug && g_cvDebug.IntValue >= 2)
            {
                LogMessage("[Lasermines Debug] Client %N became zombie, reset mine count (keeping planted mines) (behavior 2)", client);
            }
        }
    }
}
...
public void OnTouchedByEntity(const char[] output, int caller, int activator, float delay)
{
    ...
    if (g_iOwner == -1 || g_iLasermine == -1 || activator == g_iOwner || !IsClientZombie(activator))
    {
        return;
    }
...
public void OnPlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
    ...
    g_iClientLasermineCount[client] = GetClientMineCount(client);
    
    if (!IsClientZombie(client))
    {
        int mineCount = g_iClientLasermineCount[client];
        ...
        switch(g_iDeathBehavior)
        {
            case 0:
            {
                ...
                if(g_bDebug)
                {
                    LogMessage("[Lasermines Debug] Player %N spawned as human, setting mines to %d (behavior 0 - disabled)", 
                        client, g_iClientsAmount[client]);
                }
            }
            case 1:
            {
                ...
                if(g_bDebug)
                {
                    LogMessage("[Lasermines Debug] Player %N spawned as human, setting mines to %d (behavior 1 - remove on death)", 
                        client, g_iClientsAmount[client]);
                }
            }
            case 2:
            {
                if (mineCount > 0)
                {
                    ...
                    if(g_bDebug)
                    {
                        LogMessage("[Lasermines Debug] Player %N spawned as human with %d existing mines, not giving new ones (behavior 2)", 
                            client, mineCount);
                    }
                }
                else
                {
                    ...
                    if(g_bDebug)
                    {
                        LogMessage("[Lasermines Debug] Player %N spawned as human with no existing mines, setting mines to %d (behavior 2)", 
                            client, g_iClientsAmount[client]);
                    }
                }
            }
    ...
    else
    {
        if(g_bDebug)
        {
            LogMessage("[Lasermines Debug] Player %N spawned as zombie, mines set to 0", client);
        }
    }
}
...
public void OnPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
    ...
    switch(g_iDeathBehavior)
    {
        ...
        case 1:
        {
            ...
            if(g_bDebug && g_cvDebug.IntValue >= 2)
            {
                LogMessage("[Lasermines Debug] Client %N died, removed all planted mines (behavior 1)", client);
            }
        }
        case 2:
        {
            ...
            if(g_bDebug && g_cvDebug.IntValue >= 2)
            {
                LogMessage("[Lasermines Debug] Client %N died, reset mine count (keeping planted mines) (behavior 2)", client);
            }
        }
    }
}
...
public Action OnPlayerRunCmd(int client, int& buttons, int& impulse, float vel[3], float angles[3], int& weapon, int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2])
{
    ...
    if(!g_bAllowPickup || IsFakeClient(client) || !IsPlayerAlive(client) || IsClientZombie(client))
        return Plugin_Continue;
...
int PlantMine(int client, float activation_delay = 0.0, int explode_damage, int explode_radius, const int color[3] = {255, 255, 255})
{
    ...
    if(TR_DidHit(g_hTrace) && TR_GetEntityIndex(g_hTrace) < 1)
    {
        ...
        if (GetClientAuthId(client, AuthId_Steam2, steamID, sizeof(steamID)))
        {
            ...
            g_smLasermineToSteamID.SetString(key, steamID);
...
public Action OnTakeDamage(int victim, int& attacker, int& inflictor, float& damage, int& damagetype)
{
    if(ZR_IsEntityLasermine(victim))
    {
        ...
        if(1 <= attacker <= MaxClients)
        {
            ...
            if((client != -1) && (client != attacker) && !IsClientZombie(attacker))
            {
                return Plugin_Handled;
            }
...
public any Native_IsLasermine(Handle plugin, int numParams)
{
    ...
    char steamID[32];
    if(g_smLasermineToSteamID.GetString(key, steamID, sizeof(steamID)))
    {
        return true;
    }
...
public any Native_GetClientByLasermine(Handle plugin, int numParams)
{
    ...
    if(g_smLasermineToSteamID.GetString(key, steamID, sizeof(steamID)))
    {
        // Find client with this SteamID
        for (int i = 1; i <= MaxClients; i++)
        {
            if (IsClientInGame(i))
            {
                char clientSteam[32];
                if (GetClientAuthId(i, AuthId_Steam2, clientSteam, sizeof(clientSteam)) && StrEqual(steamID, clientSteam))
                {
                    return i;
                }
            }
...
public any Native_SetClientMaxLasermines(Handle plugin, int numParams)
{
    ...
    if(g_bDebug)
    {
        LogMessage("[Lasermines Debug] Native_SetClientMaxLasermines called for client %N - Setting amount to %d, marking g_iUsedByNative=true", 
            client, g_iNativeAmount);
    }
